        ...
        fetch('<?= $basePath ?>?module=ordenes&action=ajax_consultar_proveedor&term=' + encodeURIComponent(nombre.trim()))
            ...
            .then(res => {
                ...
                if (res.registrado) {
                    ...
                    if (hintProv) hintProv.innerHTML = '✨ Se autocompletaron datos de este proveedor.';
                } else {
                    if (badgeProv) {
                        badgeProv.className = 'mod-badge badge-gold';
                    ...
                    if (hdnEstado) hdnEstado.value = 'nuevo';
                    if (hintProv) hintProv.innerHTML = d ? '✨ Proveedor nuevo (primera orden). Datos autocompletados desde el directorio.' : 'ℹ️ Es la primera vez que se genera orden a este proveedor.';
                }
