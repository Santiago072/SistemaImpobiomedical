    ...
    <main class="contenido-principal">
        <div class="page-header">
            ...
            <p class="page-sub">Ítems agregados: <strong><?= $totalItems ?></strong>
                <?php if ($totalItems > 0): ?>
                | <a href="<?= $basePath ?>?module=cotizaciones&action=finalizar" class="btn btn-primary btn-sm">
        ...
        <div class="cot-grid">
            ...
            <div class="panel-form">
                <div class="card-panel">
                    <h2 class="card-title"><i class="bi bi-search"></i> Buscar Producto del Catálogo</h2>
                    <div class="search-live">
                        <input type="text" id="busquedaProducto" placeholder="Buscar por nombre..." class="input-field"
                ...
                <div class="card-panel">
                    <h2 class="card-title">
                        ...
                        <span class="badge-auto" id="badgeAuto" style="display:none">Del catálogo</span>
                    </h2>
                    ...
                    <form method="POST" enctype="multipart/form-data" action="<?= $basePath ?>?module=cotizaciones&action=crear" id="formItem">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="titulo" id="inpTitulo" class="input-field" required maxlength="255"
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" name="cantidad" id="inpCantidad" class="input-field" min="1" value="<?= $producto['cantidad'] ?? 1 ?>" required>
                            </div>
                            <div class="form-group">
                                <label>Precio Unitario (antes de IVA) *</label>
                                <input type="number" name="precio" id="inpPrecio" class="input-field" min="0" step="0.01"
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <select name="iva" id="inpIva" class="input-field" onchange="toggleIva(this.value)">
                                    <option value="si" <?= ($producto['iva'] ?? 'si') === 'si' ? 'selected' : '' ?>>Sí</option>
                                    <option value="no" <?= ($producto['iva'] ?? 'si') === 'no' ? 'selected' : '' ?>>No</option>
                                </select>
                            </div>
                            <div class="form-group" id="grupoIvaPct">
                                <label>% IVA</label>
                                <input type="number" name="porcentaje_iva" id="inpPctIva" class="input-field"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="tiempo_entrega" id="inpTiempoEntrega" class="input-field"
                                   placeholder="Ej: 5 A 15 DÍAS HÁBILES" maxlength="80">
                        </div>

                        <div class="form-group">
                            <label>Descripción *</label>
                            <textarea name="descripcion" id="inpDesc" class="input-field textarea-field"
                                      required maxlength="2000"><?= htmlspecialchars($producto['descripcion'] ?? '') ?></textarea>
                        </div>

                        <div class="form-group">
                            <label>Imagen del Producto</label>
                            <input type="file" name="foto" id="inpFoto" class="input-field" accept="image/*">
            ...
            <div class="panel-lista">
                <div class="card-panel">
                    <h2 class="card-title"><i class="bi bi-list-check"></i> Ítems de la Cotización</h2>

                    <?php if (empty($items)): ?>
                    <div class="empty-state">
                        <i class="bi bi-inbox"></i>
                        <p>Agrega ítems usando el formulario de la izquierda</p>
                    </div>
                    <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button onclick="eliminarItem(<?= $it['id'] ?>)"
                                                class="btn-icon btn-delete" title="Eliminar"><i class="bi bi-trash-fill"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
                    ...
                    <div style="margin-top:16px; text-align:right;">
                        <a href="<?= $basePath ?>?module=cotizaciones&action=finalizar" class="btn btn-primary">
...
.prev-row { display:flex; justify-content:space-between; font-size:12px; margin-bottom:4px; color:rgba(255,255,255,.7); }
.total-row { border-top:1px solid var(--glass-border); padding-top:8px; margin-top:4px; color:#fff; font-size:13px; }
.total-highlight { color: var(--amber); font-size:15px; }
.empty-state { text-align:center; padding:48px 20px; color:rgba(255,255,255,.4); }
...
function buscarProductos(q) {
    ...
    fetch(BASE + '?module=cotizaciones&action=ajax_buscar_productos&busqueda=' + encodeURIComponent(q))
        ...
        .then(json => {
            ...
            lista.innerHTML = '';
            if (!json.data.length) {
                lista.innerHTML = '<div class="sugerencia-item" style="color:rgba(255,255,255,.4)">Sin resultados</div>';
            } else {
                json.data.forEach(p => {
                    const div = document.createElement('div');
                    div.className = 'sugerencia-item';
                    div.innerHTML = `
                        ${p.foto ? `<img src="${BASE}uploads/${p.foto}">` : '<div style="width:36px;height:36px;border-radius:6px;background:rgba(255,255,255,.1);flex-shrink:0;"></div>'}
...
function autocompletar(p) {
    ...
    document.getElementById('inpIva').value           = p.iva;
    document.getElementById('inpPctIva').value        = p.porcentaje_iva || 19;
